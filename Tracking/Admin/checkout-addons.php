...
/**
 * Register custom post type for checkout addons
 */
add_action('init', 'checkout_addon_cpt');

function checkout_addon_cpt()
{
    $args = array(
        'labels'             => array(
            'name'               => __('Checkout Addons', 'woocommerce'),
            'singular_name'      => __('Checkout Addon', 'woocommerce'),
            'add_new'            => __('Add New', 'woocommerce'),
            'add_new_item'       => __('Add New Checkout Addon', 'woocommerce'),
            'edit_item'          => __('Edit Checkout Addon', 'woocommerce'),
            'new_item'           => __('New Checkout Addon', 'woocommerce'),
            'view_item'          => __('View Checkout Addon', 'woocommerce'),
            'search_items'       => __('Search Checkout Addons', 'woocommerce'),
    ...
    register_post_type('checkout-addon', $args);
}

/**
 * Add CPT meta input for product id
 */
function checkout_addon_cpt_metabox()
{
    add_meta_box(
        "checkout_addon_product_id_meta",
        "Product ID",
        "checkout_addon_meta_input",
        "checkout-addon",
        "normal",
        "low"
    );
...
add_action('admin_init', 'checkout_addon_cpt_metabox');

/**
 * Render meta input HTML
 */
function checkout_addon_meta_input()
{ ?>
    <!-- product id -->
    <p>
 ...
 */
function save_checkout_addon_cpt_meta()
{
    global $post;
    if (is_object($post)) :
...
add_action('save_post_checkout-addon', 'save_checkout_addon_cpt_meta');

/**
 * Add custom CPT columns
 */
function checkout_addon_cpt_custom_head_columns($defaults)
{
...
add_filter('manage_checkout-addon_posts_columns', 'checkout_addon_cpt_custom_head_columns', 10);

/**
 * Add custom CPT column data
 */
function checkout_addon_cpt_custom_column_data($col_name, $post_id)
{
...
add_action('manage_checkout-addon_posts_custom_column', 'checkout_addon_cpt_custom_column_data', 10, 2);

/**
 * make columns sortable
 */
add_filter('manage_edit-checkout-addon_sortable_columns', 'upsellv2_sort_checkout_addon_tracking_columns');

function upsellv2_sort_checkout_addon_tracking_columns($columns)
{
 ...
 */
add_action('pre_get_posts', 'upsellv2_checkout_addon_sortby');

function upsellv2_checkout_addon_sortby($query)
{
...
/*********************************************
 * Query checkout product addons tracking items
 *********************************************/
function upsellv2_query_checkout_addon_tracking_items()
{
 ...
 *********************************************************/
add_action('wp_ajax_nopriv_upsellv2_update_checkout_upsell_tracking', 'upsellv2_update_checkout_upsell_tracking');
add_action('wp_ajax_upsellv2_update_checkout_upsell_tracking', 'upsellv2_update_checkout_upsell_tracking');

function upsellv2_update_checkout_upsell_tracking()
{

    check_ajax_referer('update checkout product upsell tracking data');
    ...
    if (isset($_POST['clicks'])) :
        ...
        $tracked_items = upsellv2_query_checkout_addon_tracking_items();
 ...
 *********************************************/
function upsellv2_checkout_addon_tracking_js()
{ ?>
    ...
    <script>
        jQuery(document).ready(function($) {
             ...
             * REGISTER CLICKS
             ******************/
            $('.upsell-v2-checkout-addon-table-cont').click(function() {

                // retrieve product id (simple or variable)
                if ($(this).find('.upsell-v2-checkout-addon-simple-product').data('product-id')) {
                    var prod_id = $(this).find('.upsell-v2-checkout-addon-simple-product').data('product-id');
                } else {
                    var prod_id = $(this).find('.upsell-v2-checkout-addon-variable-product').data('product-id');
                }

                // retrieve nonce
                var nonce = $(document).find('#upsell-v2-checkout-addon-outer-cont').data('tracking-nonce');

                // send AJAX request
                var data = {
                    '_ajax_nonce': nonce,
                    'action': 'upsellv2_update_checkout_upsell_tracking',
                    'prod_id': prod_id,
                    'clicks': true
                }
 ...
 */
add_action('wp_footer', 'upsellv2_checkout_upsell_tracking_scripts');

function upsellv2_checkout_upsell_tracking_scripts()
{
    if (is_checkout()) :
        wp_enqueue_script('upsellv2-checkout-addon-tracking-js', upsellv2_checkout_addon_tracking_js(), ['jquery'], false, true);
    endif;
}
 ...
 */
add_filter('bulk_actions-edit-checkout-addon', function ($bulk_actions) {
 ...
 */
add_filter('handle_bulk_actions-edit-checkout-addon', function ($redirect_url, $action, $post_ids) {
