...
/**
 * Register custom post type for cart addons
 */
add_action('init', 'cart_addon_cpt');

function cart_addon_cpt()
{
    $args = array(
        'labels'             => array(
            'name'               => __('Cart Addons', 'woocommerce'),
            'singular_name'      => __('Cart Addon', 'woocommerce'),
            'add_new'            => __('Add New', 'woocommerce'),
            'add_new_item'       => __('Add New Cart Addon', 'woocommerce'),
            'edit_item'          => __('Edit Cart Addon', 'woocommerce'),
            'new_item'           => __('New Cart Addon', 'woocommerce'),
            'view_item'          => __('View Cart Addon', 'woocommerce'),
            'search_items'       => __('Search Cart Addons', 'woocommerce'),
    ...
    register_post_type('cart-addon', $args);
}

/**
 * Add CPT meta input for product id
 */
function cart_addon_cpt_metabox()
{
    add_meta_box(
        "cart_addon_product_id_meta",
        "Product ID",
        "cart_addon_meta_input",
        "cart-addon",
        "normal",
        "low"
    );
...
add_action('admin_init', 'cart_addon_cpt_metabox');

/**
 * Render meta input HTML
 */
function cart_addon_meta_input()
{ ?>
    <!-- product id -->
    <p>
 ...
 */
function save_cart_addon_cpt_meta()
{
    global $post;
...
add_action('save_post_cart-addon', 'save_cart_addon_cpt_meta');

/**
 * Add custom CPT columns
 */
function cart_addon_cpt_custom_head_columns($defaults)
{
...
add_filter('manage_cart-addon_posts_columns', 'cart_addon_cpt_custom_head_columns', 10);

/**
 * Add custom CPT column data
 */
function cart_addon_cpt_custom_column_data($col_name, $post_id)
{
...
add_action('manage_cart-addon_posts_custom_column', 'cart_addon_cpt_custom_column_data', 10, 2);

/**
 * make columns sortable
 */
add_filter('manage_edit-cart-addon_sortable_columns', 'upsellv2_sort_cart_addon_tracking_columns');

function upsellv2_sort_cart_addon_tracking_columns($columns)
{
 ...
 */
add_action('pre_get_posts', 'upsellv2_cart_addon_sortby');

function upsellv2_cart_addon_sortby($query)
{
...
/*********************************************
 * Query cart product addons tracking items
 *********************************************/
function upsellv2_query_cart_addon_tracking_items()
{
 ...
 *********************************************************/
add_action('wp_ajax_nopriv_upsellv2_update_cart_upsell_tracking', 'upsellv2_update_cart_upsell_tracking');
add_action('wp_ajax_upsellv2_update_cart_upsell_tracking', 'upsellv2_update_cart_upsell_tracking');

function upsellv2_update_cart_upsell_tracking()
{

    check_ajax_referer('update cart product upsell tracking data');
    ...
    if (isset($_POST['clicks'])) :
        ...
        $tracked_items = upsellv2_query_cart_addon_tracking_items();
 ...
 *********************************************/
function upsellv2_cart_addon_tracking_js()
{ ?>
    ...
    <script>
        jQuery(document).ready(function($) {
             ...
             * REGISTER CLICKS
             ******************/
            $('.upsell-v2-cart-addon-table-cont').click(function() {

                // retrieve product id (simple or variable)
                if ($(this).find('.upsell-v2-cart-addon-simple-product').data('product-id')) {
                    var prod_id = $(this).find('.upsell-v2-cart-addon-simple-product').data('product-id');
                } else {
                    var prod_id = $(this).find('.upsell-v2-cart-addon-variable-product').data('product-id');
                }

                // retrieve nonce
                var nonce = $(document).find('#upsell-v2-cart-addon-outer-cont').data('tracking-nonce');

                // send AJAX request
                var data = {
                    '_ajax_nonce': nonce,
                    'action': 'upsellv2_update_cart_upsell_tracking',
                    'prod_id': prod_id,
                    'clicks': true
                }
 ...
 */
add_action('wp_footer', 'upsellv2_cart_upsell_tracking_scripts');

function upsellv2_cart_upsell_tracking_scripts()
{
    if (is_cart()) :
        wp_enqueue_script('upsellv2-cart-addon-tracking-js', upsellv2_cart_addon_tracking_js(), ['jquery'], false, true);
    endif;
}
 ...
 */
add_filter('bulk_actions-edit-cart-addon', function ($bulk_actions) {
 ...
 */
add_filter('handle_bulk_actions-edit-cart-addon', function ($redirect_url, $action, $post_ids) {
