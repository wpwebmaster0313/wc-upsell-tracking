jQuery(document).ready(function ($) {
    ...
    $('.upsell-v2-checkout-addon-variable-prod-cb').each(function () {
        ...
        $('.upsell-v2-checkout-addon-variable-product-variation-select').each(function () {
        ...
        for (var key in json) {
            if (key === selected) {
                ...
                $('#variable-checkout-addon-prod-cb-' + parent_id).attr('data-variation-id', variation_id);
                $('#upsell-v2-checkout-addon-product-image-' + parent_id).attr('src', thumb_src);
                $('#upsell-v2-checkout-addon-product-image-' + parent_id).parent().attr('data-thumb-src-lg', large_src);
            }
        }
    });
    ...
    // ****************************************************************
    $('.upsell-v2-checkout-addon-variable-product-variation-select').on('change', function () {

        // retrieve parent id
        var parent_id = $(this).data('parent-id');

        // retrieve variation json from checkbox
        var json = $('#variable-checkout-addon-prod-cb-' + parent_id).data('variations');

        // variable to hold selected variation value(s)
        var selected = '';

        // traverse back up the dom tree to find all variation attribute selectors
        var v_select = $(this).parent().parent().find('select.upsell-v2-checkout-addon-variable-product-variation-select');
    ...
    $('select.upsell-v2-checkout-addon-variable-product-qty-select').change(function () {
        var qty = $(this).val();
        var parent_id = $(this).data('parent-id');
        $('#variable-checkout-addon-prod-cb-' + parent_id).attr('data-qty', qty);
    });

    // simple qty on change
    $('select.upsell-v2-checkout-addon-simple-product-qty-select').change(function () {
        var qty = $(this).val();
        var product_id = $(this).data('product-id');
        $('#simple-checkout-addon-prod-cb-' + product_id).attr('data-qty', qty);
    });
});